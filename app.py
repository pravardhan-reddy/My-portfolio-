...
from models import db, Skill, Project, Message, SiteVisit
...
def track_visits():
    """GET returns the current visit count. POST increments it by one and
    returns the new total. The frontend calls POST once per page load."""
    try:
        counter = SiteVisit.query.first()
        if not counter:
            counter = SiteVisit(count=0)
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to process visit counter', 'details': str(e)}), 500
